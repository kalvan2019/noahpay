...
import com.noahpay.pay.payment.risk.service.RiskListService;
 ...
 * @author kalvan
 */
@SysLog(group = LogGroup.RISK_LIST)
@RestController
@RequestMapping("payment/risk/risklist")
@Slf4j
public class RiskListController extends BaseController {
     ...
     */
    @PostMapping("list")
    @Permission("payment:risk:risklist:list")
    public WebResponse list(PageInfo pageInfo, @RequestParam Map<String, Object> params) {
        Page<RiskList> data = riskListService.queryPage(pageInfo, params);
     ...
     */
    @SysLog(remark = "导出风险事件记录", type = LogType.EXPORT)
    @PostMapping("download")
    @Permission("payment:risk:risklist:download")
    public void download(@RequestParam Map<String, Object> params, HttpServletResponse response) throws Exception {
        ...
        ExcelUtil.download(filename, response, RiskList.class, data, excludeColumnFiledNames);
    }
