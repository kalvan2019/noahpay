...
import com.noahpay.pay.commons.dto.platform.AppInfoImport;
...
@RestController
@RequestMapping("platform/gateway/appinfo")
@Slf4j
public class AppInfoController extends BaseController {
     ...
     */
    @PostMapping("list")
    @Permission("platform:gateway:appinfo:list")
     ...
     */
    @GetMapping("info/{id}")
    @Permission("platform:gateway:appinfo:info")
    ...
    @Permission("platform:gateway:appinfo:download")
    public void download(@RequestParam Map<String, Object> params, HttpServletResponse response) throws Exception {
        ...
        ExcelUtil.download(filename, response, AppInfo.class, data, excludeColumnFiledNames);
    }
    ...
    @PostMapping("add")
    @Permission("platform:gateway:appinfo:add")
    ...
    @Permission("platform:gateway:appinfo:upload")
    public void downloadTemplate(HttpServletResponse response) throws Exception {
        ExcelUtil.downloadTemplate(templateRootPath + "com/kalvan/platform/app信息-模板.xls", response);
    }
    ...
    @PostMapping("edit")
    @Permission("platform:gateway:appinfo:edit")
    public WebResponse edit(@Validated({Edit.class}) AppInfoImport appInfoImport) {
        if (appInfoImport.getId() == null) {
            throw new BizException(AdminReturnCode.REQUEST_ILLEGAL);
        }
        int num = appInfoService.edit(BeanUtil.toBean(appInfoImport, AppInfo.class));
    ...
    @PostMapping("delete")
    @Permission("platform:gateway:appinfo:delete")
    ...
    @Permission("platform:gateway:appinfo:updateCache")
    @PostMapping("updateCache")
    public WebResponse updateCache(String appId) {
        appInfoService.updateCache(appId);
