...
INSERT INTO `menu` ( `system_code`, `title`, `type`, `icon`, `path`, `code`, `priority`, `parent_id`, `name` )
    SELECT @systemCode,'下游对账单', 'menu', '', '/settle/merchantcheckrecord',
        'admin:settle:merchantcheckrecord:index', 1, @parentId, 'merchantcheckrecord';
...
INSERT INTO `menu` ( `system_code`,`title`, `type`, `icon`, `path`, `code`, `priority`, `parent_id` )
    SELECT @systemCode,'列表', 'button', '', '#',
     'admin:settle:merchantcheckrecord:list', 1,@newParentId;
INSERT INTO `menu` ( `system_code`,`title`, `type`, `icon`, `path`, `code`, `priority`, `parent_id` )
    SELECT @systemCode,'下载', 'button', '', '#',
     'admin:settle:merchantcheckrecord:download', 1,@newParentId;
