  ...
  <page-header-wrapper>
    <a-card :bordered="false">
          ...
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="入账状态">
                <a-input v-decorator="['state',{rules: []}]" placeholder="请输入入账状态"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
      ...
      <div class="table-operator">
        <a-button type="primary" icon="download" @click="handleDownload" v-if="hasPerm('payment:settle:merchantprofit:download')">下载</a-button>
      </div>
      <!--汇总-->
      <!--数据表格-->
      <a-table
...
<script>
  ...
  import { getList, downloadFile } from '@/api/payment/settle/merchantprofit'
  export default {
    mixins: [curdMixins],
    components: {
    },
    data () {
      this.columns = [
        ...
        {
          title: '商户实际分润',
          dataIndex: 'factProfitAmount'
        },
        {
          title: '实际入账金额',
          dataIndex: 'factIncomeAmount'
        },
        {
          title: '发票金额',
