  ...
  <page-header-wrapper>
    <a-card :bordered="false">
          ...
          <a-row :gutter="48">
            <a-col :md="8" :sm="24">
              <a-form-item label="黒名单类型">
                <a-select show-search placeholder="请选择" v-decorator="['blackType',{rules: []}]">
                  <a-select-option value="">全部</a-select-option>
                  <a-select-option v-for="(value, key) in this.black_type" :key="key">
                    ({{ key }}){{ value.dictValue }}
                  </a-select-option>
                </a-select>
              </a-form-item>
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="客户名称">
                <a-input v-decorator="['custName',{rules: []}]" placeholder="请输入客户名称"/>
              </a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
      ...
      <div class="table-operator">
        <a-button type="primary" icon="download" @click="handleDownload" v-if="hasPerm('payment:risk:blacklist:download')">下载</a-button>
        <a-button type="primary" icon="plus" @click="handleAdd" v-if="hasPerm('payment:risk:blacklist:add')">新建</a-button>
        <a-dropdown v-if="selectedRowKeys.length > 0">
          <a-menu slot="overlay">
            <a-menu-item key="delete" @click="handleMenuClick" v-if="hasPerm('payment:risk:blacklist:delete')">
              <a-icon type="delete"/>
              删除
            </a-menu-item>
          </a-menu>
        ...
        <a-button type="primary" icon="download" @click="handleDownloadTemplate" v-if="hasPerm('payment:risk:blacklist:upload')">批量导入模板</a-button>
        <a-upload
          name="file"
          :customRequest="handleUpload"
        >
          <a-button type="primary" icon="upload" v-if="hasPerm('payment:risk:blacklist:upload')">批量导入</a-button>
        </a-upload>
      </div>
      <!--汇总-->
      <!--数据表格-->
      ...
      >
        <!--字段扩展显示customRender-->
        <template v-slot:blackType="text">
          {{ dictFilter(text,'black_type') }}
        </template>
        ...
        <template v-slot:action="text, record">
          <span>
            <a href="javascript:" @click="handleEdit(record)" v-if="hasPerm('payment:risk:blacklist:edit')">编辑</a>
            <a-divider type="vertical"/>
            <a-popconfirm title="确定删除吗？" @confirm="handleDelete(record)" v-if="hasPerm('payment:risk:blacklist:delete')">
...
<script>
  ...
  import { getList, downloadFile, downloadTemplateFile, uploadFile, deleteByIds } from '@/api/payment/risk/blacklist'
      ...
      this.columns = [
        {
          title: '黒名单类型',
          dataIndex: 'blackType',
          scopedSlots: { customRender: 'blackType' }
        },
        {
          title: '客户号',
          dataIndex: 'custNo'
