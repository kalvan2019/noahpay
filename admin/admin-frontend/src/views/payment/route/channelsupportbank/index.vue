  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      <div class="table-operator">
        <a-button type="primary" icon="download" @click="handleDownload" v-if="hasPerm('payment:route:channelsupportbank:download')">下载</a-button>
        <a-button type="primary" icon="plus" @click="handleAdd" v-if="hasPerm('payment:route:channelsupportbank:add')">新建</a-button>
        <a-dropdown v-if="selectedRowKeys.length > 0">
          <a-menu slot="overlay">
            <a-menu-item key="delete" @click="handleMenuClick" v-if="hasPerm('payment:route:channelsupportbank:delete')">
              <a-icon type="delete"/>
              删除
            </a-menu-item>
          </a-menu>
        ...
        <a-button type="primary" icon="download" @click="handleDownloadTemplate" v-if="hasPerm('payment:route:channelsupportbank:upload')">批量导入模板</a-button>
        <a-upload
          name="file"
          :customRequest="handleUpload"
        >
          <a-button type="primary" icon="upload" v-if="hasPerm('payment:route:channelsupportbank:upload')">批量导入</a-button>
        </a-upload>
      </div>
      <!--汇总-->
      <!--数据表格-->
      ...
      >
        ...
        <template v-slot:bankSupport="text">
        ...
        <template v-slot:action="text, record">
          <span>
            <a href="javascript:" @click="handleEdit(record)" v-if="hasPerm('payment:route:channelsupportbank:edit')">编辑</a>
            <a-divider type="vertical"/>
            <a-popconfirm title="确定删除吗？" @confirm="handleDelete(record)" v-if="hasPerm('payment:route:channelsupportbank:delete')">
...
<script>
  ...
  import { getList, downloadFile, downloadTemplateFile, uploadFile, deleteByIds } from '@/api/payment/route/channelsupportbank'
  ...
  export default {
    ...
    data () {
      this.columns = [
        ...
        {
          title: '银行组是/非',
          dataIndex: 'bankSupport',
          scopedSlots: { customRender: 'bankSupport' }
        },
        {
          title: '备注',
          dataIndex: 'remark'
