    ...
    <a-spin :spinning="loading">
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item label="总余额">
          <a-input v-decorator="['amount', {initialValue: model.amount,rules: [{ required: true, message: '总余额不能为空！' }]}]"/>
        </a-form-item>
        <a-form-item label="冻结金额">
          <a-input v-decorator="['freezeAmount', {initialValue: model.freezeAmount,rules: [{ required: true, message: '冻结金额不能为空！' }]}]"/>
        </a-form-item>
        <a-form-item label="可用金额">
          <a-input v-decorator="['availableAmount', {initialValue: model.availableAmount,rules: [{ required: true, message: '可用金额不能为空！' }]}]"/>
        </a-form-item>
...
<script>
  ...
  import { getInfo, addInfo, editInfo } from '@/api/payment/route/channeldfpool'
  export default {
    mixins: [detailMixins],
    data () {
      return {
