    ...
    <a-spin :spinning="loading">
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item label="合作方名称">
          <a-input v-decorator="['partnerName', {initialValue: model.partnerName,rules: [{ required: true, message: '合作方名称不能为空！' }]}]"/>
        </a-form-item>
        <a-form-item label="合作方客户号">
          <a-input v-decorator="['custNo', {initialValue: model.custNo,rules: [{ required: true, message: '合作方客户号不能为空！' }]}]"/>
        </a-form-item>
...
<script>
  ...
  import { getInfo, addInfo, editInfo } from '@/api/payment/cust/partnerinfo'
  export default {
    mixins: [detailMixins],
    data () {
      return {
