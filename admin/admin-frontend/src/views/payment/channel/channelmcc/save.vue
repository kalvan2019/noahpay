      ...
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item label="行业类别">
          <a-select
            show-search
            placeholder="请选择"
            v-decorator="['mcc', {initialValue: model.mcc,rules:[{ required: true, message: '行业类别不能为空！' }]}]">
            <a-select-option v-for="(value, key) in this.mcc" :key="parseInt(key)">
              ({{ key }}){{ value.dictValue }}
            </a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="渠道类目">
          <a-input v-decorator="['channelMcc', {initialValue: model.channelMcc,rules: [{ required: true, message: '渠道类目不能为空！' }]}]"/>
        </a-form-item>
...
<script>
  ...
  import { getInfo, addInfo, editInfo } from '@/api/payment/channel/channelmcc'
  export default {
    mixins: [detailMixins],
    data () {
      return {
