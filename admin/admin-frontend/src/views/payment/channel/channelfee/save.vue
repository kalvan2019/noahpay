    ...
    <a-spin :spinning="loading">
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item label="固定收费">
          <a-input v-decorator="['fixFee', {initialValue: model.fixFee,rules: []}]"/>
        </a-form-item>
        <a-form-item label="最低手续费">
          <a-input v-decorator="['minFee', {initialValue: model.minFee,rules: [{ required: true, message: '最低手续费不能为空！' }]}]"/>
        </a-form-item>
...
<script>
  ...
  import { getInfo, addInfo, editInfo } from '@/api/payment/channel/channelfee'
  export default {
    mixins: [detailMixins],
    data () {
      return {
