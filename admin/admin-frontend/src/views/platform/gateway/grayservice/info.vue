  ...
  >
    <a-card :bordered="true">
      <a-descriptions title="">
        ...
        <a-descriptions-item label="服务实例">{{ model.serviceId }}</a-descriptions-item>
        <a-descriptions-item label="ip">{{ model.host }}</a-descriptions-item>
        <a-descriptions-item label="端口">{{ model.port }}</a-descriptions-item>
      </a-descriptions>
      <a-divider style="margin-bottom: 32px"/>
      <a-descriptions title="">
        <a-descriptions-item label="规则名">{{ model.ruleName }}</a-descriptions-item>
        <a-descriptions-item label="灰度标签">{{ model.grayTag }}</a-descriptions-item>
...
<script>
  ...
  import { getInfo } from '@/api/platform/gateway/grayservice'
  export default {
