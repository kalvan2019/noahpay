  ...
  <page-header-wrapper>
    <a-card :bordered="false">
          ...
          <a-row :gutter="48">
            <a-col :md="8" :sm="24">
              <a-form-item label="appId">
                <a-select show-search :placeholder="$t('global.placeholder.select')" v-decorator="['appId',{rules: []}]">
                  <a-select-option value="">{{ $t('global.show.all') }}</a-select-option>
                  <a-select-option v-for="data in this.all_app" :key="data.appId">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="服务">
                <a-select show-search :placeholder="$t('global.placeholder.select')" v-decorator="['service',{rules: []}]">
                  <a-select-option value="">{{ $t('global.show.all') }}</a-select-option>
      ...
      <div class="table-operator">
        <a-button type="primary" icon="download" @click="handleDownload" v-if="hasPerm('platform:gateway:authorize:download')">{{ $t('global.btn.download') }}</a-button>
        <a-button type="primary" icon="plus" @click="handleAdd" v-if="hasPerm('platform:gateway:authorize:add')">{{ $t('global.btn.add') }}</a-button>
        <a-button type="primary" @click="handleUpdateCache" v-if="hasPerm('platform:gateway:authorize:updateCache')">更新缓存</a-button>
        <a-dropdown v-if="selectedRowKeys.length > 0">
          <a-menu slot="overlay">
            <a-menu-item key="delete" @click="handleMenuClick" v-if="hasPerm('platform:gateway:authorize:delete')">
              <a-icon type="delete"/>
              {{ $t('global.btn.delete') }}
            </a-menu-item>
          </a-menu>
        ...
        <a-button type="primary" icon="download" @click="handleDownloadTemplate" v-if="hasPerm('platform:gateway:authorize:upload')">{{ $t('global.btn.template') }}</a-button>
        <a-upload
          name="file"
          :customRequest="handleUpload"
        >
          <a-button type="primary" icon="upload" v-if="hasPerm('platform:gateway:authorize:upload')">{{ $t('global.btn.upload') }}</a-button>
        </a-upload>
      </div>
      <!--Sum-->
      <!--Data Table-->
      ...
      >
        <!--CustomRender-->
        <template v-slot:state="text, record" v-if="hasPerm('platform:gateway:authorize:edit')">
          <a>
        ...
        <template v-slot:action="text, record">
          <span>
            <a href="javascript:" @click="handleInfo(record)" v-if="hasPerm('platform:gateway:authorize:info')">{{ $t('global.btn.info') }}</a>
            <a-divider type="vertical"/>
            <a-dropdown v-if="hasPerm('platform:gateway:authorize:edit') || hasPerm('platform:gateway:authorize:delete') || hasPerm('platform:gateway:authorize:updateCache')">
              <a class="ant-dropdown-link">
                {{ $t('global.btn.more') }} <a-icon type="down" />
              </a>
              <a-menu slot="overlay">
                <a-menu-item v-if="hasPerm('platform:gateway:authorize:edit')">
                  <a href="javascript:" @click="handleEdit(record)">{{ $t('global.btn.edit') }}</a>
                </a-menu-item>
                <a-menu-item v-if="hasPerm('platform:gateway:authorize:delete')">
                  <a-popconfirm :title="$t('global.confirm.delete')" @confirm="handleDelete(record)">
                    <a-icon slot="icon" type="question-circle-o" style="color: red"/>
                    <a href="javascript:">{{ $t('global.btn.delete') }}</a>
                  </a-popconfirm>
                </a-menu-item>
                <a-menu-item v-if="hasPerm('platform:gateway:authorize:updateCache')">
...
<script>
  ...
  import { getList, downloadFile, downloadTemplateFile, uploadFile, deleteByIds, updateCache, editInfo, getAllService, getAllApp } from '@/api/platform/gateway/authorize'
